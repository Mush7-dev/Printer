...
const UsersList = ({ users, onPrintImage, loading }) => {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Գտնվել է {users.length} օգտատեր</Text>
        ...
        >
          <View style={styles.printContent}>
            ...
            {users.map((user, index) => {
              const userId = user.id || user.customerId || user.mNumber;
              const displayName = user.name || user.customerName || user.fullName || 'Անանուն օգտատեր';
              ...
              const displayAddress = user.address || 
                `${user.streetName || ''} ${user.building || ''} ${user.apartment || ''}`.trim();
              ...
              return (
                <View key={userId || index} style={styles.printUserSection}>
                  ...
                  {displayAddress && (
                    <Text style={styles.printableText}>   Հասցե: {displayAddress}</Text>
                  )}
                  <Text style={styles.printableText}>
                    Գումար: {price} դրամ
                  </Text>
            ...
            <Text style={styles.printableText}>
              Ընդհանուր գումար: {users.reduce((sum, user) => {
                const userId = user.id || user.customerId || user.mNumber;
          ...
          return (
            <View key={userId || index} style={styles.userCard}>
              <View style={styles.userInfo}>
                ...
                {displayAddress && (
                  <Text style={styles.userDetail}>Հասցե: {displayAddress}</Text>
                )}
              </View>
      ...
      <View style={styles.printButtonContainer}>
        <Button text="Տպել բոլորը" onPress={handlePrintAll} loading={loading} />
      </View>
    </View>
  );
};
